...
vi.mock("next/navigation", () => ({
  usePathname: () => "/dashboard/comments",
...
describe("comments/page.tsx 数据加载失败态", () => {
  ...
  it("加载失败时展示错误条与「重试」，不渲染空态", async () => {
    ...
    expect(container.textContent).toContain("加载评论数据失败");
    expect(container.textContent).toContain("重试");
    expect(container.textContent).not.toContain("暂无评论数据");
  });
  ...
  it("点击「重试」重新拉取，成功后渲染空态", async () => {
    ...
    expect(container.textContent).not.toContain("加载评论数据失败");
    expect(container.textContent).toContain("暂无评论数据");
  });
});
