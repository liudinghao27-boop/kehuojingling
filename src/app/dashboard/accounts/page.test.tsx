...
describe("accounts/page.tsx 数据加载失败态", () => {
  ...
  it("加载失败时展示错误条与「重试」，不渲染空态", async () => {
    ...
    // accounts 页沿用文件惯例用 getErrorMessage 展示错误原文
    expect(container.textContent).toContain("network down");
    expect(container.textContent).toContain("重试");
    expect(container.textContent).not.toContain("暂无账号");
  });
  ...
  it("点击「重试」重新拉取，成功后渲染空态", async () => {
    ...
    fetchMock.mockImplementation(async (url: string) => {
      if (url.startsWith("/api/user/sender-accounts/stats"))
        return okJson({
          stats: {
            total: 0,
            active: 0,
      ...
      if (url.startsWith("/api/user/sender-accounts")) return okJson({ accounts: [] });
      throw new Error(`unexpected url: ${url}`);
    });
    await clickRetry();

    expect(container.textContent).not.toContain("network down");
    expect(container.textContent).toContain("暂无账号");
  });
});
